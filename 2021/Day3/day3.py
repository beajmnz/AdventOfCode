# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 08:20:28 2021
...
diagnostics_file = "./adventofcode.com_2021_day_3_input.txt"
diagnostics = pd.read_csv(diagnostics_file, dtype="str", header=None, names=["diag"])
diagnostics = diagnostics["diag"].apply(lambda x: pd.Series(list(x))).astype(int)

gamma = []
epsilon = []

for col in diagnostics.columns:
    gamma.append(1 if diagnostics[col].sum() > len(diagnostics) / 2 else 0)
    epsilon.append(0 if diagnostics[col].sum() > len(diagnostics) / 2 else 1)
gamma_int = int("".join(str(x) for x in gamma), base=2)
epsilon_int = int("".join(str(x) for x in epsilon), base=2)

print("power comsumption = " + str(gamma_int * epsilon_int))
...
ogr_df = diagnostics.copy()

for col in ogr_df.columns:
    if len(ogr_df) == 1:
        break
    ogr_df = ogr_df[ogr_df[col] == int(ogr_df[col].sum() >= len(ogr_df) / 2)]
ogr_int = int("".join(str(x) for x in ogr_df.iloc[0]), base=2)


csr_df = diagnostics.copy()

for col in csr_df.columns:
    if len(csr_df) == 1:
        break
    csr_df = csr_df[csr_df[col] == int(csr_df[col].sum() < len(csr_df) / 2)]
csr_int = int("".join(str(x) for x in csr_df.iloc[0]), base=2)

print("life support = " + str(ogr_int * csr_int))
